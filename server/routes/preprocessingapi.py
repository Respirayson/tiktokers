...
class SelectFeaturesHandler(Resource):
    def get(self):
        return jsonify(
            {"status": "success", "message": "Select Features API connected."}
        )
...
class DropColumns(Resource):
    def get(self):
        return jsonify(
            {"message": "Drop Columns API connected.", "status": "successful"}
        )
    ...
    def post(self):
        ...
        if not os.path.exists(file_path):
            return (
                jsonify({"message": "File not found", "status": "unsuccessful"}),
                404,
            )
